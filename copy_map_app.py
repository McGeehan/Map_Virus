...
def open_google_maps(address): #create a function that takes an address argument
    ...
    #This line sets the base url for Google Maps directions. The 'destination' query parameter will be appended to this base url
    destination = address.replace(" ", "+")
    #this line replaces spaces in the address with '+' character to encode the address for use in a URL
    url = base_url + destination
    #this line constructs the full URL by concatenating the base URL and the encoded destination address
...
while True:
    ...
    #this line calls the google maps function with the new address as an argument which opens Google Maps
    ...
    time.sleep(5)
    #this line pauses the program for 5 seconds before checking the clipboard again... hopefully helps prevent the program from excessive CPU resources
